...
interface Today {
  ...
  cash_in_hand: string;
  bank_balance: string;
  money_in_today: string;
  money_out_today: string;
  gold_in_hand_g: string;
  gold_with_workers_g: string;
  silver_in_hand_g: string;
  silver_with_workers_g: string;
  stones_with_workers_ct: string;
  customer_receivable: string;
  supplier_payable: string;
  worker_payable: string;
  metal_owed_to_makers_g: string;
      ...
      {t && (
        <>
          ...
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-5">
            ...
            <Metric
              label="Money today"
              value={fmtMoney(t.money_in_today)}
              // In and out on one tile, because a day that took a lot and paid
              // out more is not a good day and two separate tiles let it look
              // like one.
              sub={`${fmtMoney(t.money_out_today)} out`}
              to="/cash"
            />
            <Metric
              label="Cash in hand"
              value={fmtMoney(t.cash_in_hand)}
              sub={`${fmtMoney(t.bank_balance)} in bank`}
          ...
          {/* --- what the shop is holding ---
              Gold and silver on separate lines and never added: they differ a
              hundredfold in value, so a combined "metal" figure is a number in
              no unit at all. Stones stay in carats for the same reason. */}
          <section>
            <h2 className="eyebrow">What you're holding</h2>
            <div className="mt-2 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              ...
              />
              <Metric
                label="Silver in the safe"
                value={g3(t.silver_in_hand_g)}
                sub={
                  Number(t.silver_with_workers_g)
                    ? `${g3(t.silver_with_workers_g)} out with workers`
                    : undefined
                }
                to="/stock"
              />
              <Metric
                ...
                value={
                  Number(t.stones_with_workers_ct)
                    ? `${Number(t.stones_with_workers_ct).toFixed(2)} ct`
                    : "—"
                }
                sub="stones they could not produce"
                to="/vendors"
              />
              <Metric
                label="You owe workers"
                value={fmtMoney(t.worker_payable)}
                sub={
                  Number(t.metal_owed_to_makers_g)
                    ? `and ${g3(t.metal_owed_to_makers_g)} of metal`
                    : undefined
                }
                tone={Number(t.metal_owed_to_makers_g) > 0 ? "bad" : undefined}
                to="/vendors"
              />
            </div>
          </section>
          ...
          {/* --- what is on the floor ---
              The age matters more than the count: a count cannot tell a busy
              stage from a stuck one, and stuck is the only reason to look. */}
          {data.floor.length > 0 && (
            <section>
              <h2 className="eyebrow">On the floor</h2>
              <div className="card mt-2 p-0">
                <ul className="divide-y divide-slate-100">
                  {data.floor.map((f) => (
                    <li
                      key={f.department_id ?? f.department}
                      className="flex items-baseline gap-3 px-5 py-2.5"
                    >
                      <Link
                        to={`/designs?current_department_id=${f.department_id ?? ""}`}
