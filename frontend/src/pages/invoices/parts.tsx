...
export interface Invoice {
  ...
  invoice_no: string;
  sale_type: string;
  // Who the sale is credited to, by name. Null on a walk-in nobody in
  // particular served, which is most bills.
  seller_id: number | null;
  seller_name: string | null;
  customer_name: string | null;
  // Which of the shop's two bills this is. A finished piece is billed on its
  // metal; a parcel of loose stones has no gold on it at all and argues its
  // discount against the stone price instead of the gold weight — so the two
  // carry different columns rather than one carrying blanks.
  kind: "finished_product" | "loose_material";
  status: string;
  customer_id: number;
